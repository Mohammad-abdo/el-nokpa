...
import { getAllOffers, getProducts } from "../api/api";
...
const BestSellsOne = () => {
  ...
  const getProductsNewOffers = async () => {
    try {
                      ...
                      <div className="countdown" id={`countdown-${product.id}`}>
                        <ul className="countdown-list style-three flex-align flex-wrap">
                          <li className="countdown-list__item text-heading flex-align gap-4 text-sm fw-medium">
                            {timeLeft?.hours ?? "00"} Hours
                          </li>
                          <li className="countdown-list__item text-heading flex-align gap-4 text-sm fw-medium">
                            {timeLeft?.minutes ?? "00"} Min
                          </li>
                          <li className="countdown-list__item text-heading flex-align gap-4 text-sm fw-medium">
                            {timeLeft?.seconds ?? "00"} Sec
                          </li>
                        </ul>
                      </div>
                    </div>
                      ...
                      <div className="product-card__price mb-16">
                        {product.discount > 0 && (
                          <span className="text-gray-400 text-md fw-semibold text-decoration-line-through">
                            $
                            {(
                              parseFloat(product.pack_sizes[0]?.price || 0) *
                              (1 + parseFloat(product.discount) / 100)
                            ).toFixed(2)}
                          </span>
                        )}
                        <span className="text-heading text-md fw-semibold ">
                          ${product.pack_sizes[0]?.price ?? "0.00"}{" "}
